...
OnKeyPressed() ; A function that sends a keystroke to OBS when a keyboard key is pressed/the mouse has moved
{
	Critical
	SetKeyDelay, 10
	SetTitleMatchMode, 2 ; Makes it so WinTtile can be a substring of the window title
	ControlSend, ahk_parent, {Blind}{F1}, OBS ; 'F1' can be changed to whatever key you want
}
